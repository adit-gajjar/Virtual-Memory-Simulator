...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// swap out frame and update status bits
		int new_swap_off = swap_pageout(frame, coremap[frame].pte->swap_off);
		coremap[frame].pte->swap_off = new_swap_off;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	//addr_t location = PGTBL_INDEX(vaddr);



	// check if page directory entry is initialized. if not, call init_second_level
	...
	if (p->frame & PG_VALID){ // p is valid, therefore there is a hit
		hit_count++;
	} else {	// if not valid check swap bit.
		miss_count++;
		/*
		 If the entry is invalid and on swap, then a (simulated) physical frame
 		should be allocated and filled by reading the page data from swap.
		*/
		...
		if (p->frame & PG_ONSWAP){
			int swap_into = allocate_frame(p);
			swap_pagein(swap_into, p->swap_off);
			swap_into = swap_into << PAGE_SHIFT;
			swap_into = swap_into | PG_VALID;
			p->frame = swap_into;

		} else { //swap not valid. allocate a blank frame
			int new_location = allocate_frame(p);
			init_frame(new_location, vaddr);
			new_location = (new_location << PAGE_SHIFT);
			new_location = new_location | PG_VALID;
			p->frame = new_location;
		}
